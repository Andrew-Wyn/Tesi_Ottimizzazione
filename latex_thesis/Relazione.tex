...
\SetAlgoLined
\KwIn{Manifold $M$; funzione costo $f$ differenziabile definita su $M$; retrazione $R$; trasporto vettoriale $\mathcal{T}$; scalare $L > 0$; punto di partenza $x^0 \in M$}
\KwOut{Sequenza $\{x^k\}$}
$\gamma_0 \leftarrow 1$, $l=0$\;
\For{$k=0, 1, 2, ...$}{
  $H_0^k \leftarrow \gamma_kid$\;
  $q = \nabla^(R) f(x^k)$\;
    \For{$i=k-1, k-2, ..., l$}{
        $\alpha_i = p_i$ $\langle s_i,q \rangle_{x^k}$\;
        $q \leftarrow q - \alpha_i y_i$\;
    }
    $z \leftarrow H_0^kq$
    \For{$i=l, l+1, ..., k-1$}{
        $\beta = p_i$ $\langle y_i,z \rangle_{x^k}$\;
        $z \leftarrow z + s_i(\alpha_i - \beta)$\;
    }
    $\eta_k \leftarrow -z$\;
    $x^{k+1} \leftarrow R_{x^k}(\eta_k)$\;
    Definire: $s_k = \mathcal{T}_{x^k \to x^{k+1}}(\eta_k)$, $y_k = \frac{\nabla^{(R)} f(x^{k+1})}{\sigma_k} - \mathcal{T}_{x^k \to x^{k+1}}(\nabla^(R) f(x^k))$, $p_k = \frac{1}{\langle s_k,y_k \rangle_{x^{k+1}}}$, $\gamma_{k+1} = \frac{\langle s_k,y_k \rangle_{x^{k+1}}}{\parallel y_k \parallel^2}$, $\sigma_k = \frac{\parallel \eta_k \parallel}{\parallel \mathcal{T}_{x^k \to x^{k+1}}(\eta_k) \parallel}$;
    $l = max\{k-l, 0\}$\;
    Aggiungere $s_k$, $y_k$ e $p_k$ in uno storage e se $k > l$, allora scartare $s_{l-1}$, $y_{l-1}$ e $p_{l-1}$ dallo storage\;
    Trasportare $s_l, s_{l+1}, ..., s_{k-1}$ e $y_l, y_{l+1}, ..., y_{k-1}$ da $T_{x^k}M$ a $T_{x^{k+1}}M$ tramite il trasporto vettoriale $\mathcal{T}$\;
}
\end{algorithm}
\end{center}
Nella nostra implementazione dell'algoritmo \textbf{L-BFGS} abbiamo deciso di non applicare valori di riscalamento, tramite ricerca lineare esatta o non, alla direzione $\eta_k$, per le stesse motivazioni riportate nella sotto-sezione precedente.
...
\bibitem{A Broyden Class of Quasi-Newton Methods for Riemannian Optimization} 
