...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{Varietà $M$; funzione costo $f$ differenziabile definita su $M$; retraction $R$; scalari $\bar{\alpha} > 0$, $c, \beta, \sigma \in (0,1)$; punto iniziale $x^0 \in M$}
\KwOut{Sequenza $\{x^k\}$}
\For{$k=0, 1, 2, ...$}{
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{Varietà $M$; funzione costo $f$ differenziabile definita su $M$; retrazione $R$; scalare $\alpha > 0$; punto iniziale $x_0 \in M$, condizione di arresto $\varepsilon$}
\KwOut{Sequenza $\{x_k\}$}
\For{$k=0, 1, 2, ...$}{
    $g_k \leftarrow \nabla^{(R)} f(x_k)$;\\
    \If{$\| g_k \| < \varepsilon$}{
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{Varietà $M$; funzione costo $f$ differenziabile definita su $M$; retrazione $R$; scalari $\gamma \in (0,1)$, $\sigma \in (0,1)$, $\lambda > 0$; punto iniziale $x_0 \in M$, condizione di arresto $\varepsilon$}
\KwOut{Sequenza $\{x_k\}$}
\For{$k=0, 1, 2, ...$}{
    $g_k \leftarrow \nabla^{(R)} f(x_k)$;\\
    \If{$\| g_k \| < \varepsilon$}{
    ...
    \[f(R_{x_k}(\sigma^h\lambda d_k)) \leq f(x_k) + \gamma \sigma^h \lambda \langle g_k, d_k \rangle_{x_k};\]\\
...
\begin{algorithm}[H]
\SetAlgoLined
\KwIn{Varietà $M$; funzione costo $f$ differenziabile definita su $M$; retrazione $R$ trasporto vettoriale $\mathcal{T}$; scalari $\alpha_{\max} > \alpha_{\min} > 0$, $\alpha_0^{BB} \in [\alpha_{\min}, \alpha_{\max}]$; punto di partenza $x_0 \in M$}
\KwOut{Sequenza $\{x_k\}$}
$g_0 \leftarrow \nabla^{(R)} f(x_0)$.\\
\For{$k=0, 1, 2, ...$}{
    \If{$\| g_k \| < \varepsilon$}{
    ...
    \begin{equation*}
    \begin{cases}
        \alpha_{k+1}^{BB} = \min\{\alpha_{\max}, \max\{\tau_{k+1}, \alpha_{\min}\}\}\} \quad se \langle s_k,y_k \rangle_{x_{k+1}} > 0;\\
        \alpha_{k+1}^{BB} = \alpha_{\max} \quad altrimenti;
    \end{cases}
    \end{equation*}
}
