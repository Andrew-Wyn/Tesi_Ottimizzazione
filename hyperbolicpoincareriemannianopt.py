...
TODO: 
* scrivere armijo euclidean/riemannian, implementare solamente poincare euclideo
* plot funzione costo e gradiente (Done!)
* implementare algoritmo (Done!)
...
def lambda_x(x, c=1):
  """
  Compute the conformal factor 𝜆𝑐𝑥 for a point on the ball
...
def geodesic_unit(t, x, v, c=1):
  """
  Unit speed geodesic starting from x with direction v/‖v‖x
  """
...
def poincare_log(x, y, c=1):
  """
  Logarithmic map for two points 𝑥 and 𝑦 on the manifold.
  """
  a = mobius_add(-x, y, c)
...
def hyperboloid_gradient(theta, x_set):
  ...
  return mink_grad + minkowski_dot(theta, mink_grad) * theta
...
def hyperbolic_log(theta_1, theta_2):
  return hyperboloid_dist(theta_1, theta_2)*(theta_2-minkowski_dot(theta_1, theta_2)*theta_1)/(np.dot(theta_2-minkowski_dot(theta_1, theta_2)*theta_1, theta_2-minkowski_dot(theta_1, theta_2)*theta_1))


def hyperboloid_transport(theta_1, theta_2, v):
  return v # identity transport
  # return v - (minkowski_dot(hyperbolic_log(theta_1, theta_2), v)/hyperboloid_dist(theta_1, theta_2)**2)*(hyperbolic_log(theta_1, theta_2) + hyperbolic_log(theta_2, theta_1))
...
seq = poincare_barycenter_iannazzo(x_set_complex, 0.2, 100)
...
psi_seq, f_seq, g_seq = optimisation_fl_poincare_euclidean(psi_0, 0.0004, x_set, 100)
print("Limit sequence poincare euclideo: ", psi_seq[-1])
plot_seq(psi_seq, f_seq, g_seq)

psi_seq, f_seq, g_seq = optimisation_fl_hyperboloid(psi_0, 0.2, x_set, 40)
...
psi_seq, f_seq, g_seq = armijo_opt_poincare_riemannian(psi_0, x_set, 0.5, 0.8, 0.001, 400)
print("Limit sequence poincare: ", psi_seq[-1])
plot_seq(psi_seq, f_seq, g_seq)

psi_seq, f_seq, g_seq = armijo_opt_hiper_riemannian(psi_0, x_set, 0.5, 0.3, 0.9, 100)
print("Limit sequence iperboloide: ", psi_seq[-1])
plot_seq(psi_seq, f_seq, g_seq)

"""### Barzilai-Borwein"""

def non_monotic_armijo_hyperboloid(k, theta, x_set_h, g_k, a_BB, sigma, gamma, M, f_k_seq):
  h = 0
  while True:
    max_l = -np.inf
    for j in range(min(k+1, M)):
      l = f_k_seq[k+1-(j+1)] - gamma*(sigma**h)*a_BB*minkowski_dot(g_k, g_k)
      if l > max_l:
        max_l = l
    if frechet_mean(hyperboloid_exp(theta, -(sigma**h)*a_BB*g_k), x_set_h, hyperboloid_dist) <= max_l:
      return h
    h += 1

def RBB_NMLS_hyperboloid(psi_0, x_set, sigma, gamma, M, a_min, a_max, max_steps=10):
  psi_seq = [psi_0]
  f_seq = []
  g_seq = []
  ...
  for k in range(max_steps):
    ...
    h = 0#non_monotic_armijo_hyperboloid(k, theta, x_set_h, g_k, a_BB, sigma, gamma, M, f_k_seq)
    a_k = sigma**h*a_BB
...
def non_monotic_armijo_poincare(k, psi, x_set, g_k, a_BB, sigma, gamma, M, f_k_seq):
  h = 0
  while True:
    max_l = -np.inf
    for j in range(min(k+1, M)):
      ...
      if l > max_l:
        max_l = l
    if frechet_mean(poincare_exp(psi, -(sigma**h)*a_BB*g_k), x_set, poincare_dist) <= max_l - gamma*(sigma**h)*a_BB*(lambda_x(psi)**2*np.dot(g_k, g_k)):
      return h
    h += 1

def RBB_NMLS_poincare(psi_0, x_set, sigma, gamma, M, a_min, a_max, max_steps=10):
  psi_seq = [psi_0]
  f_seq = []
  g_seq = []
  ...
  for k in range(max_steps):
    ...
    h = 0#non_monotic_armijo_poincare(k, psi, x_set, g_k, a_BB, sigma, gamma, M, f_k_seq)
    a_k = sigma**h*a_BB
    ...
    y_k = new_g + s_k/a_k
    new_tau = riemannian_norm(s_k, s_k)/riemannian_norm(s_k, y_k)

    if riemannian_norm(s_k, y_k) > 0:
      a_BB = min(a_max, max(a_min, new_tau))
    else:
      a_BB = a_max
  return psi_seq, f_seq, g_seq

psi_seq, f_seq, g_seq = RBB_NMLS_poincare(psi_0, x_set, 0.5, 0.5, 10, 0.0001, 0.001, 100)
print("Limit sequence poincare: ", psi_seq[-1])
plot_seq(psi_seq, f_seq, g_seq)

# supporta un learnign rate piu alto convergendo in modo piu veloce
psi_seq, f_seq, g_seq = RBB_NMLS_hyperboloid(psi_0, x_set, 0.5, 0.5, 10, 0.01, 0.3, 100)
print("Limit sequence iperboloide: ", psi_seq[-1])
plot_seq(psi_seq, f_seq, g_seq)